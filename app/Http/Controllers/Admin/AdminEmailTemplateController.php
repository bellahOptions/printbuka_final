...
class AdminEmailTemplateController extends Controller
{
    ...
    {
        $registry = config('printbuka_email_templates', []);
    ...
    public function update(Request $request, string $key): RedirectResponse
    {
        ...
        return redirect()
            ...
            ->with('status', '"'.$entry['name'].'" template saved.');
    }
    ...
    {
        // The registry is keyed by dotted strings (e.g. 'staff.kyc_reminder'), so this
        // deliberately avoids config('printbuka_email_templates.'.$key) — the config()
        // helper's dot-notation would split that into nested array lookups instead of
        // a single flat key.
        $entry = config('printbuka_email_templates', [])[$key] ?? null;
        abort_if($entry === null, 404, 'Unknown email template.');
