...
<footer class="relative bg-slate-950 text-white overflow-hidden">
    ...
    {{-- Glow blobs --}}
    <div class="pointer-events-none absolute -top-40 -left-40 w-[500px] h-[500px] rounded-full bg-brand-700/10 blur-3xl"></div>
    <div class="pointer-events-none absolute -bottom-40 -right-40 w-[400px] h-[400px] rounded-full bg-violet-700/8 blur-3xl"></div>

    {{-- ===== MAIN WIDGETS ===== --}}
    <div class="relative pb-container pt-16 pb-10">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-8">
            ...
            <div>
                ...
                <div class="space-y-4">
                    <div class="flex items-start gap-3">
                        <div class="w-9 h-9 rounded-lg bg-brand-600/20 border border-brand-600/30 flex items-center justify-center shrink-0 mt-0.5">
                            <x-heroicon-o-map-pin class="w-4 h-4 text-brand-400" />
                        </div>
                        <div>
                    ...
                    <div class="flex items-start gap-3">
                        <div class="w-9 h-9 rounded-lg bg-brand-600/20 border border-brand-600/30 flex items-center justify-center shrink-0 mt-0.5">
                            <x-heroicon-o-phone class="w-4 h-4 text-brand-400" />
                        </div>
                        <div>
                            ...
                            <a href="tel:{{ preg_replace('/\s+/', '', explode(',', $phone)[0]) }}"
                               class="text-sm font-semibold text-slate-200 hover:text-brand-400 transition-colors leading-snug">
                                {{ $phone }}
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start gap-3">
                        <div class="w-9 h-9 rounded-lg bg-brand-600/20 border border-brand-600/30 flex items-center justify-center shrink-0 mt-0.5">
                            <x-heroicon-o-envelope class="w-4 h-4 text-brand-400" />
                        </div>
                        <div>
                            <p class="text-xs font-bold text-slate-500 uppercase tracking-wide mb-0.5">Email</p>
                            <a href="mailto:{{ $email }}"
                               class="text-sm font-semibold text-slate-200 hover:text-brand-400 transition-colors">
                                {{ $email }}
                            </a>
                        </div>
                    </div>
            ...
            <div>
                <div class="mb-6">
                    <h3 class="text-base font-black text-white relative inline-block pb-3 after:content-[''] after:absolute after:left-0 after:bottom-0 after:w-8 after:h-0.5 after:bg-brand-600 after:rounded-full">
                        Useful Links
                    </h3>
                </div>
                <ul class="space-y-3">
                    <li>
                        <a href="{{ route('home') }}"
                           class="flex items-center gap-2 text-sm text-slate-400 hover:text-brand-400 hover:translate-x-1 transition-all group">
                            <x-heroicon-o-chevron-right class="w-3.5 h-3.5 text-brand-600/60 group-hover:text-brand-400 shrink-0" />
                            Home
                        </a>
                    </li>
                    <li>
            ...
            <div>
                ...
                <form action="#" method="POST" class="mb-7" onsubmit="return false;">
                    <div class="flex rounded-xl overflow-hidden border border-white/10 bg-white/5 focus-within:border-brand-600/60 transition-colors">
                        <input type="email"
                               placeholder="Your email address"
                               class="flex-1 bg-transparent text-sm text-white placeholder-slate-500 px-4 py-3 outline-none min-w-0" />
                        <button type="submit"
                                class="bg-brand-600 hover:bg-brand-700 text-white px-4 py-3 transition-colors shrink-0 flex items-center justify-center"
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <a href="{{ $siteSettings['social_facebook'] }}"
                           ...
                           class="w-9 h-9 rounded-lg bg-white/5 border border-white/10 hover:bg-brand-600 hover:border-brand-600 flex items-center justify-center transition-all"
                           aria-label="Facebook">
                        ...
                        @endif
                        @if($siteSettings['social_whatsapp'] ?? null)
