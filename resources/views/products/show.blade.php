...
@section('content')
    ...
    <main class="bg-white text-slate-900">
        <section class="bg-slate-50 py-16">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div class="grid gap-10 lg:grid-cols-[1.05fr_0.95fr] items-start">
                    <div class="space-y-6">
                        <a href="{{ route('products.index') }}" class="text-sm font-black uppercase tracking-[0.24em] text-brand-700 transition hover:text-brand-800">Back to catalog</a>
                        <div class="inline-flex items-center gap-3 rounded-full bg-white px-4 py-2 text-sm font-black text-slate-900 shadow-sm ring-1 ring-slate-200">
                            <span>MOQ</span>
                            <span class="rounded-full bg-brand-100 px-3 py-1 text-brand-700">{{ $product->moq }}</span>
                        </div>
                        <div class="space-y-3 rounded-[2rem] bg-white p-8 shadow-xl ring-1 ring-slate-200">
                            <div>
                                <p class="pb-eyebrow text-cyan-700">{{ $product->category?->name ?? 'Print product' }}</p>
                                <h1 class="pb-display mt-4 text-4xl sm:text-5xl">{{ $product->name }}</h1>
                            </div>
                            ...
                            <div class="grid gap-3 sm:grid-cols-2">
                                <div class="rounded-3xl bg-slate-50 p-5">
                                    ...
                                    <p class="mt-3 text-2xl font-black text-brand-700">{{ $product->hasAvailablePrice() ? 'NGN '.number_format($product->price, 2) : 'Contact us' }}</p>
                                </div>
                                <div class="rounded-3xl bg-slate-50 p-5">
                        ...
                        <div class="rounded-[2rem] bg-white p-8 shadow-xl ring-1 ring-slate-200">
                            ...
                            <ul class="mt-6 space-y-4 text-sm leading-6 text-slate-600">
                                <li class="flex gap-3"><span class="mt-1 inline-flex h-8 w-8 items-center justify-center rounded-2xl bg-brand-100 text-brand-700">✓</span> Clear print quality for every run.</li>
                            ...
                            <div class="space-y-4">
                                <div class="flex items-center justify-between gap-4">
                                    <div>
                                        ...
                                        <p class="mt-2 text-3xl font-black text-brand-700">{{ $product->hasAvailablePrice() ? 'NGN '.number_format($product->price, 2) : 'Contact us' }}</p>
                                    </div>
                                    <span class="rounded-full bg-brand-50 px-4 py-2 text-sm font-black text-brand-700">MOQ {{ $product->moq }}</span>
                                </div>
                                ...
                                @if ($product->hasAvailablePrice())
                                    <a href="{{ route('orders.create', $product) }}" class="pb-cta-primary w-full h-12 rounded-3xl">Start order</a>
                                @else
                                    <a href="{{ $product->quoteRequestUrl() }}" class="pb-cta-primary w-full h-12 rounded-3xl">Request quotation</a>
                                @endif
            ...
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div class="grid gap-6 lg:grid-cols-3">
                    <article class="rounded-[2rem] border border-slate-200 bg-white p-7 shadow-sm">
                        <p class="text-sm font-black uppercase tracking-[0.24em] text-brand-700">Step 1</p>
        ...
        @if ($relatedProducts->isNotEmpty())
            <section class="bg-slate-50 py-16">
                <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    <p class="pb-eyebrow">More Products</p>
                    <h2 class="pb-display mt-2 text-3xl sm:text-4xl">You may also need</h2>
                    <div class="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
                        @foreach ($relatedProducts as $relatedProduct)
                            <a href="{{ route('products.show', $relatedProduct) }}" class="rounded-[1.75rem] border border-slate-200 bg-white p-5 transition hover:-translate-y-1 hover:shadow-lg">
                                ...
                                <p class="mt-4 text-lg font-black text-brand-700">{{ $relatedProduct->hasAvailablePrice() ? 'NGN '.number_format($relatedProduct->price, 2) : 'Contact us' }}</p>
                            </a>
                        @endforeach
                    </div>
                </div>
