    ...
    <main class="bg-[#f4fbfb] px-4 py-14 text-slate-900 sm:px-6 lg:px-8">
        <section class="mx-auto max-w-5xl space-y-8">
            <div class="rounded-2xl bg-slate-950 p-8 text-white shadow-xl shadow-cyan-950/20 sm:p-10">
                ...
                <h1 class="pb-display mt-5 text-4xl leading-tight sm:text-5xl">{{ $title }}</h1>
            ...
            <article class="rounded-2xl border border-slate-200 bg-white p-7 shadow-sm sm:p-10">
                @if ($rawContent !== '')
                    <div class="prose prose-slate max-w-none prose-headings:font-black prose-a:text-brand-700 prose-a:no-underline hover:prose-a:underline">
                        {!! $renderedContent !!}
                    </div>
                @else
            ...
            <div class="flex flex-wrap gap-3">
                <a href="{{ route('policies.terms') }}" class="inline-flex min-h-11 items-center justify-center rounded-md border border-slate-200 bg-white px-4 text-sm font-black text-slate-800 transition hover:border-brand-300 hover:text-brand-700">Terms</a>
                <a href="{{ route('policies.privacy') }}" class="inline-flex min-h-11 items-center justify-center rounded-md border border-slate-200 bg-white px-4 text-sm font-black text-slate-800 transition hover:border-brand-300 hover:text-brand-700">Privacy</a>
                <a href="{{ route('policies.refund') }}" class="inline-flex min-h-11 items-center justify-center rounded-md border border-slate-200 bg-white px-4 text-sm font-black text-slate-800 transition hover:border-brand-300 hover:text-brand-700">Refund</a>
            </div>
        </section>
    </main>
@endsection
