...
<form method="GET" action="{{ route('admin.inventory.index') }}" class="pb-card p-4 mb-6 flex flex-wrap gap-3 items-end">
    ...
    @if($filters['search'] !== '' || $filters['category'] !== '' || $filters['status'] !== '' || $filters['itemType'] !== '')
        <a href="{{ route('admin.inventory.index') }}" class="btn btn-ghost font-black text-slate-500">Clear</a>
    @endif
    @if($filters['status'] !== '')
                ...
                @forelse($items as $item)
                    <tr>
                        ...
                        <td class="text-right whitespace-nowrap">
                            <a href="{{ route('admin.inventory.show', $item) }}" class="btn btn-ghost btn-sm font-bold">View</a>
                            @if(auth()->user()?->canAdmin('inventory.manage'))
                                <a href="{{ route('admin.inventory.edit', $item) }}" class="btn btn-ghost btn-sm font-bold">Edit</a>
                            @endif
                        </td>
                    </tr>
                @empty
