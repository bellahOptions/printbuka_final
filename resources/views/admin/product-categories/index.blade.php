    ...
    <div class="mx-auto max-w-7xl">
            ...
            <div class="mt-8 overflow-x-auto rounded-md border border-slate-200 bg-white shadow-sm">
                <table class="w-full min-w-[760px] text-left text-sm">
                    <thead><tr class="border-b border-slate-200 bg-slate-50 text-xs font-black uppercase tracking-wide text-slate-500"><th class="px-5 py-4">Name</th><th class="px-5 py-4">Tag</th><th class="px-5 py-4">Products</th><th class="px-5 py-4">Status</th><th class="px-5 py-4"></th></tr></thead>
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($categories as $category)
                            <tr><td class="px-5 py-4 font-black">{{ $category->name }}</td><td class="px-5 py-4">{{ $category->tag }}</td><td class="px-5 py-4">{{ $category->products_count }}</td><td class="px-5 py-4">{{ $category->is_active ? 'Active' : 'Hidden' }}</td><td class="px-5 py-4 text-right"><a href="{{ route('admin.product-categories.edit', $category) }}" class="font-black text-pink-700">Edit</a><form action="{{ route('admin.product-categories.destroy', $category) }}" method="POST" class="inline">@csrf @method('DELETE') <button class="ml-4 font-black text-slate-500 hover:text-red-700">Delete</button></form></td></tr>
                        @empty
