...
@section('content')
    <main class="bg-[#f4fbfb] px-4 py-16 text-slate-900 sm:px-6 lg:px-8">
        <section class="mx-auto max-w-4xl rounded-md bg-white p-6 text-center shadow-xl shadow-cyan-950/10 sm:p-10">
            <p class="mx-auto inline-flex rounded-md bg-brand-50 px-4 py-2 text-sm font-black uppercase tracking-wide text-brand-700">Service Order Submitted</p>
            <h1 class="pb-display mt-5 text-5xl">We have your {{ $service['name'] }} request.</h1>
            ...
            <div class="mx-auto mt-8 grid max-w-2xl gap-4 text-left sm:grid-cols-2">
                ...
                <div class="rounded-md border border-slate-200 p-5">
                    ...
                    <p class="mt-1 text-2xl font-black text-brand-700">NGN {{ number_format((float) ($order->invoice?->total_amount ?? $order->total_price), 2) }}</p>
                </div>
            </div>

            <div class="mt-8 flex flex-wrap justify-center gap-3">
                <a href="{{ route('services.show', $service['slug']) }}" class="rounded-md border border-slate-200 px-6 py-3 text-sm font-black text-slate-800 transition hover:border-brand-300 hover:text-brand-700">Back to Service</a>
                <a href="{{ route('orders.track.show', $order) }}" class="rounded-md border border-slate-200 px-6 py-3 text-sm font-black text-slate-800 transition hover:border-brand-300 hover:text-brand-700">Track This Order</a>
