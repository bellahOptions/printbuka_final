                ...
                @forelse ($products as $product)
                    <article class="rounded-md border border-slate-100 bg-white p-4 shadow-sm">
                        ...
                        <div class="mt-2 flex flex-wrap gap-3 text-xs text-gray-500">
                            ...
                            <span>Price: {{ $product->hasAvailablePrice() ? 'NGN '.number_format($product->price, 2) : 'Contact us' }}</span>
                            <span>{{ $product->paper_size }}</span>
                            <span>{{ $product->paper_density }}</span>
                        </div>
                        <a href="{{ $product->hasAvailablePrice() ? route('orders.create', $product) : route('products.show', $product) }}" class="mt-3 inline-flex rounded-md bg-slate-950 px-4 py-2 text-xs font-black text-white transition hover:bg-pink-700">{{ $product->hasAvailablePrice() ? 'Start Order' : 'View' }}</a>
                    </article>
                @empty
