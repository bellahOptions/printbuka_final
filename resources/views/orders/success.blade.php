...
@section('content')
    <main class="bg-[#f4fbfb] px-4 py-16 text-slate-900 sm:px-6 lg:px-8">
        <section class="mx-auto max-w-4xl rounded-md bg-white p-6 text-center shadow-xl shadow-cyan-950/10 sm:p-10">
            <p class="pb-eyebrow justify-center">Order Submitted</p>
            <h1 class="pb-display mt-5 text-4xl sm:text-5xl">We have your request.</h1>
            ...
            <div class="mx-auto mt-8 grid max-w-2xl gap-4 text-left sm:grid-cols-2">
                ...
                <div class="rounded-md border border-slate-200 p-5">
                    ...
                    <p class="pb-price mt-1 text-2xl text-brand-700">NGN {{ number_format($order->invoice?->total_amount ?? $order->total_price, 2) }}</p>
                </div>
            ...
            <div class="mt-8 flex flex-wrap justify-center gap-3">
                <a href="{{ route('products.index') }}" class="pb-cta-outline">Browse More Products</a>
                <a href="{{ route('orders.track.show', $order) }}" class="pb-cta-outline">Track This Order</a>
                <a href="{{ route('home') }}" class="pb-cta-primary">Back Home</a>
            </div>
        </section>
    </main>
@endsection
