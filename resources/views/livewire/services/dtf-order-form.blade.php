<div class="pb-28 md:pb-0">
    <p class="text-sm font-black uppercase tracking-wide text-cyan-700">DTF Order Form</p>
    <h2 class="mt-2 text-4xl text-slate-950">Place Your DTF Order</h2>
    <p class="mt-3 text-sm leading-6 text-slate-600">Pricing updates instantly based on film size, design support, and delivery choice. Material is fixed to Film.</p>
    ...
    <form wire:submit="submit" class="mt-6 grid gap-6 lg:grid-cols-[1fr_300px]">
        <div class="space-y-5">
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                <label class="text-sm font-black text-slate-800">Material
                    <input type="text" value="{{ $material }}" readonly class="mt-2 min-h-12 w-full rounded-md border border-slate-200 bg-slate-100 px-4 text-sm font-semibold text-slate-600">
                </label>
                <label class="text-sm font-black text-slate-800 sm:col-span-2">Film Size
                    <select wire:model.live="film_size" class="mt-2 min-h-12 w-full rounded-md border border-slate-200 px-4 text-sm font-semibold outline-none transition focus:border-pink-500 focus:ring-4 focus:ring-pink-100">
                        @foreach ($filmSizeOptions as $option)
                            <option value="{{ $option['label'] }}">{{ $option['label'] }} (+ NGN {{ number_format((float) $option['price'], 2) }})</option>
                        @endforeach
                    </select>
                    @error('film_size') <span class="mt-1 block text-xs font-bold text-red-600">{{ $message }}</span> @enderror
                </label>
            </div>
            ...
            <button type="submit" wire:loading.attr="disabled" wire:target="submit" class="hidden md:inline-flex min-h-12 items-center justify-center rounded-md bg-pink-600 px-6 text-sm font-black text-white transition hover:bg-pink-700 disabled:cursor-not-allowed disabled:opacity-70">Submit Order</button>
        </div>
        ...
        <aside class="hidden rounded-md border border-slate-200 bg-white p-5 shadow-sm lg:block lg:sticky lg:top-24 lg:self-start">
            ...
            <div class="mt-4 space-y-2 text-xs font-semibold text-slate-600">
                ...
                <div class="flex items-center justify-between"><span>Film size</span><span>NGN {{ number_format((float) $this->filmSizePrice, 2) }}</span></div>
        ...
        <div class="fixed inset-x-0 bottom-0 z-40 border-t border-slate-200 bg-white/95 p-3 backdrop-blur md:hidden">
            <div class="mx-auto flex max-w-7xl items-center justify-between gap-3">
                ...
                <button type="submit" wire:loading.attr="disabled" wire:target="submit" class="inline-flex min-h-11 items-center justify-center rounded-md bg-pink-600 px-5 text-sm font-black text-white transition hover:bg-pink-700 disabled:cursor-not-allowed disabled:opacity-70">Submit Order</button>
            </div>
        </div>
    </form>
</div>
